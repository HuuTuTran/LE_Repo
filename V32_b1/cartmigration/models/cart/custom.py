...
class LeCartCustom(LeBasecart):
	...
	def display_import_source(self):
		...
		queries = {
			...
			# 'reviews': {
			# 	'type': 'select',
			# 	'query': "SELECT COUNT(1) AS count FROM reviews WHERE reviews_id > " + to_str(self._notice['process']['reviews']['id_src']),
			# },
		}
	...
	def get_categories_ext_export(self, categories):
		...
		#self.log(categories_ext["data"]["categories_description"][2]["categories_name"],"aReturn")
	...
	def convert_category_export(self, category, categories_ext):
		...
		category_data['images'] = [category['categories_image']]
		...
		category_data['created_at'] = str(datetime.fromtimestamp(to_int(category['date_added'])) if category['date_added'] else '0000-00-00 00:00:00')
		category_data['updated_at'] = str(datetime.fromtimestamp(to_int(category['last_modified'])) if category['last_modified'] else '0000-00-00 00:00:00')
		category_data['description'] = 'this is a description'
		for i in categories_ext["data"]["categories_description"]:
			if i["categories_id"] == category['categories_id']:
				category_data['name'] = i["categories_name"]
				break
		category_data["languages"] = categories_ext["data"]["languages"][0]
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['created_at'] = str(datetime.fromtimestamp(to_int(product['products_date_added'])) if product['products_date_added'] else get_current_time())
		product_data['updated_at'] = str(datetime.fromtimestamp(to_int(product['products_last_modified'])) if to_int(product['products_last_modified']) else product_data['created_at'])

		for i in products_ext_data["products_description"]:
			if i["products_id"] == product['products_id']:
				product_data['name'] = html.escape(i['products_name'])
				product_data['description'] = html.escape(i['products_description'])
				break
		...
		# 	meta_keywords=get_row_from_list_by_field(product_description,'products_id',1)
		# 	meta_description = get_row_from_list_by_field(product_description, 'products_id', 2)
		# 	meta_title = get_row_from_list_by_field(product_description, 'products_id', 3)
		# 	if meta_keywords:
		...
		# # product_images = get_list_from_list_by_field(product['products_image'], 'products_id',product['products_id'])#select product_image where products_id = value
		# # if product_images:
		# # 	if check_thumbnail:
		# # 		for product_image in product_images:
		# # 			image_data = get_row_from_list_by_field(product['products_image'], 'products_id',product_image['image_id'])
		...
		# # 		for product_image in product_images:
		# # 			if i == 0 :
		# # 				image_data = get_row_from_list_by_field(products_ext_data['images'], 'products_id', product_image['image_id'])
		# # 				product_data['thumb_image']['url'] = url_product_image
		...
		# # if special:
		# # product_data['special_price']['price'] = special['products_price']
		# # product_data['special_price']['start_date'] = special['specials_date_added']
		# # product_data['special_price']['end_date'] = special['products_date_available']
		...
		product_data['manufacturer']['name'] = "No manufacturer name"

		#product_data['images'].append(product['products_image'])  
		...
		#self.log(self._notice['src']['languages_select'],"self._notice['src']['languages_select']")
	...
	def get_customers_ext_export(self, customers):
		...
		# self.log(customers_ext,"customers_ext")
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		customer_data['email'] = html.escape(customer['customers_email_address'])
		...
		customer_data['dob'] = str(customer['customers_dob'])
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_data['subtotal']['amount'] = get_value_by_key_in_dict(order, 'order_total', 0.0000)
		...
		order_data['updated_at'] = str(order["last_modified"])
		order_data['currency'] = order['currency']
		...
		order_customer['first_name'] = order['billing_name'].split(" ")[0]
		order_customer['last_name'] = order['billing_name'].split(" ")[1]
		...
		if customer_state:
			...
			customer_address['state']['code'] = customer_state['zone_code']
		...
		billing_name = self.get_name_from_string(order['billing_name'].split(" ")[0] + ' '+ order['billing_name'].split(" ")[1])
		...
		if billing_state:
			...
			order_billing['state']['code'] = billing_state['zone_code']
		...
		delivery_name = self.get_name_from_string(order['delivery_name'].split(" ")[0] + ' ' + order['delivery_name'].split(" ")[1])
		...
		delivery_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'code', order['delivery_state'])
		if delivery_state:
		...
		for order_product in order_products:
			...
			order_item['product']['id'] = order_product['products_lsfs_item_id']
